...
int N, M;
long long a, b;
long long T_max[4 * 100002];
long long T_min[4 * 100002];
long long arr[100002];
const int MAX = 1e9 + 10;

void init_max(int node, int st, int ed)
{
  if (st == ed)
  {
    T_max[node] = arr[st];
  ...
  {
    init_max(node * 2, st, (st + ed) / 2);
    init_max(node * 2 + 1, (st + ed) / 2 + 1, ed);
...
long long query_max(int node, int st, int ed, int L, int R)
{
  if (L > ed || R < st)
  {
    return 0;
  ...
  return max(query_max(node * 2, st, (st + ed) / 2, L, R), query_max(node * 2 + 1, (st + ed) / 2 + 1, ed, L, R));
}
